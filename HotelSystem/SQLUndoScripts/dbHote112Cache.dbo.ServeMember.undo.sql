...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeMember] DROP CONSTRAINT [DF_ServeMember_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeMember] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeMember] DROP CONSTRAINT [DF_ServeMember_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeMember] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ServeMember_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[ServeMember_Tombstone];
