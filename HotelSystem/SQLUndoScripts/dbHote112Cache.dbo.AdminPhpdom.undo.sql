...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[AdminPhpdom] DROP CONSTRAINT [DF_AdminPhpdom_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[AdminPhpdom] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[AdminPhpdom] DROP CONSTRAINT [DF_AdminPhpdom_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[AdminPhpdom] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AdminPhpdom_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[AdminPhpdom_Tombstone];
