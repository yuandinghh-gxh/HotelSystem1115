...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsSell] DROP CONSTRAINT [DF_GoodsSell_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsSell] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsSell] DROP CONSTRAINT [DF_GoodsSell_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsSell] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GoodsSell_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[GoodsSell_Tombstone];
