...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsClass] DROP CONSTRAINT [DF_GoodsClass_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsClass] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsClass] DROP CONSTRAINT [DF_GoodsClass_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[GoodsClass] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GoodsClass_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[GoodsClass_Tombstone];
