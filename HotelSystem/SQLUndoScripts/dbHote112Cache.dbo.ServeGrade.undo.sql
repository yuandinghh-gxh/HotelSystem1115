...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeGrade] DROP CONSTRAINT [DF_ServeGrade_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeGrade] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeGrade] DROP CONSTRAINT [DF_ServeGrade_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[ServeGrade] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ServeGrade_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[ServeGrade_Tombstone];
