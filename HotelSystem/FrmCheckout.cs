...
namespace HotelSystem1115
{
 ...
 //      private DataTable RentRoomall = new DataTable(); //旅馆 总 房间
        ...
        private int  count, yearincome, monincome, sel;  //年月日收入
   //     private int cDestine;
    ...
    //       fm = _fm;
        ...
        private void FrmDayinquire_Load(object sender, EventArgs e)
        {
            ...
            sel = 0;                                    //选择所有
            comboBox3.Text = "选择所有";
            comboBox3.SelectedIndex = 0;  //第一次显示 

            Clientdt = SqlHelp.ExcuteAsAdapter("select * from Users");
            DataRow row = Clientdt.NewRow();                     //前面加一个空白 行
            row["UserId"] = 0;
            row["LoginName"] = "选择所有";
            Clientdt.Rows.InsertAt(row, 0);
            comboBox1.DataSource = Clientdt;  //加载宾客来源
            comboBox1.DisplayMember = "LoginName";
            comboBox1.ValueMember = "UserId";
            RentRoom = SqlHelp.ExcuteAsAdapter("select * from RentRoom where OrderState = '已结账'");
            count = 0;
            allReceivable = 0;   //总收入
            Showitem();
        }
                ...
                foreach (DataRow r in RentRoom.Rows) //取得年月日 营业额
                {
                    ...
                    //{
                        ...
                        if (sel == 0 || lstr == selitem)
                        {
                            ...
                            item.Text = r["Settlenum"].ToString().Trim();    //RoomName
                            item.SubItems.Add(r["RoomName"].ToString().Trim()); //RoomTypeName
                            lstr = r["GuestName"].ToString();
                            item.SubItems.Add(lstr);
                            lstr = r["Deposit"].ToString();   //押金
                            item.SubItems.Add(lstr);
                            item.SubItems.Add(r["RentCost"].ToString());  //应收金额
                            lstr = r["RentCost"].ToString();   //押金
                            sql = r["RentRoomOrder"].ToString();   //实收
                            int i = int.Parse(lstr)-int.Parse(sql);
                            lstr = i.ToString();
                            item.SubItems.Add(lstr);  //优惠金额  
                            item.SubItems.Add(r["RentRoomOrder"].ToString());  //实收金额
    ...
    //            }
      ...
      //    throw;
            ...
            sql = count.ToString(CultureInfo.InvariantCulture);
