...
namespace Serial_Comm_Tester {
	public partial class COMForm : Form {  // public Conversion_Table conTable = null;
		...
		private int tXChartCount = 0;   //		bool tabpage2set = true;
	#endregion
		public COMForm( ) {
			InitializeComponent();
		...
		{
			Invoke( (MethodInvoker)delegate ( ) { comBoBoxformatText = comboBoxDecodeFormat.Text; } );
		...
		private async void btnRead_Click(object sender, EventArgs e) {   //手动读
			if (serialPort1.IsOpen) {
				try {
					SerialEncoding();         //comm 解码
					...
					if (s != "") {
						...
						RXcounter++;
						lblRxSent.Update();
						lblRxSent.Text = "RX :" + RXcounter;    						//flash up rx
						RX = true;
					}
					//this is the original///////////////////////////
					//richTextBox2.Text += serialPort1.ReadExisting();  					// serialPort1.DiscardInBuffer();
				} catch (Exception ex) {
					ComPortClosed();
		...
		public void btnOpenPort_Click(object sender, EventArgs e) {   //打开串行口
			try {
				if (comboBoxActiveComPorts.Text == "" || comboBoxBaudRate.Text == "" || comboBoxParity.Text == "" || comboBoxDataBits.Text == "" || comboBoxStopBits.Text == "" || comboBoxFlow.Text == "" || comboBoxDecodeFormat.Text == "" && !serialPort1.IsOpen) {
					...
					progressBar1.Value = 50;                    //finally it opens the serial port here if its not open
					serialPort1.Open();
		...
		private async void btnClosePort_Click(object sender, EventArgs e) {  //Close COM
			if (serialPort1.IsOpen == true) {
				ckBAppendLogs.Enabled = true; ckBEnableLogs.Enabled = true;
				ckBOverwriteLogs.Enabled = true;
				if (comboBoxFlow.Text == "RTS/CTS") {
					serialPort1.RtsEnable = false; textBoxRTS.BackColor = Color.Red;
					// serialPort1.RtsEnable = false;  //---------------------------------------17/6/17
					textBoxRTS.Text = "Rts Off";
				}
				richTextBoxSend.Enabled = false; btnRefreshComPorts.Enabled = true;
				if (comboBoxFlow.Text == "None" | comboBoxFlow.Text == "XOnXOff") {
				...
				chkBAutoRead.Enabled = false;
				checkBoxAutoSend.Enabled = false;
				chkBAutoReadDec.Checked = false;                //         chkBAutoReadHex.Checked = false;
				chkBAutoReadDec.Enabled = false;                //         chkBAutoReadHex.Enabled = false;
				...
				//checkBoxSendBinary.Enabled = false;  				//checkBoxSendOct.Enabled = false;
				//modified 24/01/17 disable COMBOboxes
		...
		public async Task Button1ClickAsync( ) {
			...
			if (serialPort1.IsOpen)
				try {
					...
					//richTextBox2.Enabled = false;					//richTextBox3.Enabled = false;					//richTextBox4.Enabled = false;
					//MODIFIED 24/01/17 wont clear boxes untill re opened
					//richTextBoxDec.Text = "";  					//richTextBoxHex.Text = "";					//richTextBoxSend.Text = "";
					//MODIFIED 24/01/17 LABEL INSTEAD  					//  richTextBox2.Text = "Serial Port Is Closed";
					lblPortStatus.Text = "Port is Closed";
					lblPortStatus.BackColor = Color.Red;

					textBoxRI.Text = ""; textBoxRI.BackColor = Color.LightSkyBlue; textBoxBI.BackColor = Color.LightSkyBlue;
					textBoxBI.Text = ""; progressBar1.Value = 0;
					btnSend.Enabled = false; btnRead.Enabled = false;
					btnClosePort.Enabled = false; btnOpenPort.Enabled = true;
					//rts dtr buttons
					btnRtsOn.Enabled = true; btnDtrOn.Enabled = true;
					btnRtsOff.Enabled = false; btnDtrOff.Enabled = false;
					chkBAutoRead.Checked = false; checkBoxAutoSend.Checked = false;
					chkBAutoRead.Enabled = false; checkBoxAutoSend.Enabled = false;
					chkBAutoReadDec.Checked = false;
					//            chkBAutoReadHex.Checked = false;
					chkBAutoReadDec.Enabled = false;
					//           chkBAutoReadHex.Enabled = false;
					chkBConvertToHexDec.Enabled = false; chkBConvertToHexDec.Checked = false;
					checkBoxSendHex.Enabled = false; checkBoxSendHex.Checked = false;
					checkBoxSendDec.Enabled = false; checkBoxSendDec.Checked = false;
					checkBoxSendNormal.Enabled = false; checkBoxSendNormal.Checked = false;
					//checkBoxSendBinary.Enabled = false;					//checkBoxSendOct.Enabled = false;
		...
		private void ComPortClosed( ) {
			...
			lblPortStatus.Text = "Port is Closed";
			lblPortStatus.BackColor = Color.Red;            //  richTextBoxSend.Text = "";
			textBoxRI.Text = "";
			...
			checkBoxSendDec.Enabled = false; checkBoxSendDec.Checked = false;
			checkBoxSendNormal.Enabled = false; checkBoxSendNormal.Checked = false;
			checkBox2DEC.Enabled = false; checkBox2DEC.Checked = false; checkBox3DEC.Enabled = false;
			checkBox3DEC.Checked = false;
			//checkBoxSendBinary.Enabled = false;			//checkBoxSendOct.Enabled = false;			//modified 24/01/17 disable COMBOboxes
			comboBoxActiveComPorts.Enabled = true;
			comboBoxBaudRate.Enabled = true;
			comboBoxDataBits.Enabled = true;
			comboBoxDecodeFormat.Enabled = true;
		...
		private string comBoBoxformatText;    //编码类型
		...
		private async void SendData(object sender, EventArgs e) {
			if (serialPort1.IsOpen && sendDataNoError == true) {
				...
				if (send_repeat_counter < (int)send_repeat.Value) {
					tx_data = richTextBoxSend.Text;                     //progressBar1.Value = send_repeat_counter;		//progressBar1.Update();
					try {
						...
						if (checkBoxSendHex.Checked == true) {
							...
							if (boolCarrigeReturn == true) {
								serialPort1.Write( asciiToHexSend2 + "\r" );								//  TX = true;
							}
							if (boolCarrigeReturn == false && boolCarrigeReturnLF == false) {
								//  SerialEncoding();
								serialPort1.Write( asciiToHexSend2 );
		...
		private void btnSend_Click(object sender, EventArgs e) {   //发送COM SHJ  >>>>>>>bool decsend
			...
			sendDataNoError = true; richTextBoxSend.Focus();
		...
		public async void serialPort1_DataReceived(object sender, SerialDataReceivedEventArgs e) {    //this allows access to the value of comboBox1 cross threading
			Invoke( (MethodInvoker)delegate ( ) {	comBoBoxformatText = comboBoxDecodeFormat.Text;	} );
			SerialEncoding();
		...
		private void btnClearAllText_Click(object sender, EventArgs e) {
			...
			lblRxSent.Text = "RX :" + RXcounter;
			lblTxSent.Text = "TX :" + TXcounter;
			//if (serialPort1.IsOpen) 			//{			//    serialPort1.DiscardInBuffer();			//    serialPort1.DiscardOutBuffer();			//}
		}
			...
			if (serialPort1.IsOpen)
				try {
					if (serialPort1.IsOpen && checkBoxAutoSend.Checked) {
						...
						// char[] ch = new char[1];						// ch[0] = e.KeyChar;						//serialPort1.Write(ch, 0, 1);
		...
		public string toDec(string inp2) {
			...
			foreach (char L in value) {
				int v2 = Convert.ToInt32( L );                  //long v2 = Convert.ToInt64(L);
		...
		private void richTextBox2_TextChanged(object sender, EventArgs e) {
			...
			if (serialPort1.IsOpen && chkBConvertToHexDec.Checked && richTextBoxRecieve.Text != "") {
				...
				//richTextBoxHex.Text += toHex(richTextBoxRecieve.Text);				//  richTextBoxHex.Text = ConvertHex(richTextBoxRecieve.Text);				//  richTextBoxDec.Text += toDec(richTextBoxRecieve.Text);
			}
		}
		...
		private async void timer2_Tick(object sender, EventArgs e) {
			...
			if (richTextBoxRecieve.Lines.Length > 5000 | richTextBoxHex.Lines.Length > 5000 | richTextBoxDec.Lines.Length > 5000 && stopMessage == false) {
				...
				chkBAutoReadHex.Checked = false;				stopMessage = true;
		...
		private void button1_Click(object sender, EventArgs e) {    //校验和计算
																	// Conversion_Table conTable = new Conversion_Table();
